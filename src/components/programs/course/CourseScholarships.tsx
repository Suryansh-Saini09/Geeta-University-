...
export default function ScholarshipsSection({ scholarships }: CourseScholarshipsProps = {}) {
  const title = scholarships?.scholarships?.title || scholarshipData.title;
  const description = scholarships?.scholarships?.description || scholarshipData.description;
  const criteria = scholarships?.scholarships?.criteria
    ? scholarships.scholarships.criteria.map((c) => ({ title: c }))
    : scholarshipData.criteria;
  const buttonText = scholarships?.scholarships?.linkText || scholarshipData.buttonText;
  const buttonHref = scholarships?.scholarships?.linkUrl || scholarshipData.buttonHref;

  const gutsTitle = scholarships?.guts?.title || gutsData.title;
  const gutsDescription = scholarships?.guts?.description || gutsData.description;
